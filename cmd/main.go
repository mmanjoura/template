...
func main() {
	...
	template.Version = strings.TrimPrefix(version, "")
...
func (m *Main) ParseFlags(ctx context.Context, args []string) error {
	...
	// home directory. This is a template task as configuration files are typing
...
func (m *Main) Run(ctx context.Context) (err error) {
	...
	if m.Config.Rollbar.Token != "" {
		...
		rollbar.SetServerRoot("github.com/mmanjoura/template")
		template.ReportError = rollbarReportError
		template.ReportPanic = rollbarReportPanic
	...
	// ShopService := sqlite.NewShopService(m.DB)
	// ShopMembershipService := sqlite.NewShopMembershipService(m.DB)
...
func rollbarReportError(ctx context.Context, err error, args ...interface{}) {
	if template.ErrorCode(err) != template.EINTERNAL {
		return
	}
